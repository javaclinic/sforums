...
public class TopicFormController {
    ...
    @Autowired
    public TopicFormController(TopicDao topicDao, ForumDao forumDao, UserContextService userContextService) {
    ...
    public void initBinder(WebDataBinder binder) {
        binder.registerCustomEditor(Forum.class, new IdentifiableEntityEditor(this.forumDao));
    ...
    public String processSubmit(@ModelAttribute("topic") @Valid Topic topic, BindingResult result, SessionStatus status) {
        if (!result.hasErrors()) {
            try {
                this.topicDao.save(topic);
                status.setComplete();
                return "redirect:topic.html?id=" + topic.getId() + "&success=true";
